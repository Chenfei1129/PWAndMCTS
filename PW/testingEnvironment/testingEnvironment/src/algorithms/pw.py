...
class InitializeChildren:
    def __init__(self, actionSpace, transition, getActionPrior, pw):
...
'''
        if self.pw(node, actionNode):
                numChild = len(actionNode.children)
                allNextStates = [actionNode.children[i].id.values() for i in range(numChild)]
                if nextState not in allNextStates: 
                    nextStateNode = Node(parent=actionNode, id={action: nextState}, numVisited=0, sumValue=0,
                      isExpanded=False)
'''
class Expand:
...
class SelectAction:
    def __init__(self, calculateScore, pw):
...
class SelectNextState:
    def __init__(self, selectAction, pw):
